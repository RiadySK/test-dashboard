...
export const reservationApi = {
  ...
  async create(data: CreateReservationRequest): Promise<{ ReservationID: number, confirmationNumber: string, guestName: string }> {
  ...
  // Update reservation
  async update(id: number, data: { notes?: string, stays?: Array<any> }): Promise<DetailedReservation> {
    const response = await fetch(`${API_BASE}/reservations/${id}`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json',
  ...
  // Check-in a guest
  async checkIn(id: number, data: any): Promise<{ message: string, reservation: DetailedReservation }> {
